...
/**
 * Test access restrictions for Customer graphs
 *
 * Class CustomerAccessTest
 * @package Tests\Services\Grapher\Graph
 */
class CustomerApiAccessTest extends Access
{
    ...
    public function testApiPublicAccess()
    {
        ...
        $response = $this->get('/grapher/customer?id=2');
    ...
    public function testWebOwnUserCustUserAccess()
    {
        ...
        // customer user
        $response = $this->actingAs( $this->getCustUser( 'hecustuser' ) )->get('/grapher/customer?id=2');
        $response->assertStatus(200);

        // customer admin user
        $response = $this->actingAs( $this->getCustAdminUser( 'hecustadmin' ) )->get('/grapher/customer?id=2');
        $response->assertStatus(200);

        // non-customer user
        $response = $this->actingAs( $this->getCustUser( 'imcustuser' ) )->get('/grapher/customer?id=2');
        $response->assertStatus(403);

        // non-customer admin user
        $response = $this->actingAs( $this->getCustAdminUser( 'imcustadmin' ) )->get('/grapher/customer?id=2');
        $response->assertStatus(403);

        // superadmin
        $response = $this->actingAs( $this->getSuperUser() )->get('/grapher/customer?id=2');
        $response->assertStatus(200);
    }
    ...
    public function testWebCustUserAccess()
    {
        ...
        $response = $this->actingAs( $this->getCustUser() )->get('/grapher/customer?id=2');
        $response->assertStatus(200);

        $response = $this->actingAs( $this->getCustAdminUser() )->get('/grapher/customer?id=2');
        $response->assertStatus(200);
