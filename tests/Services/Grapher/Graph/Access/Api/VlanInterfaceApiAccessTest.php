...
/**
 * Test access restrictions for VlanInterface graphs
 *
 * Class VlanInterfaceAccessTest
 * @package Tests\Services\Grapher\Graph
 */
class VlanInterfaceApiAccessTest extends Access
{
    ...
    public function testApiPublicAccess()
    {
        ...
        $response = $this->get('/grapher/vlaninterface?id=1');
    ...
    public function testWebOwnUserCustUserAccess()
    {
        ...
        // customer user
        $response = $this->actingAs( $this->getCustUser( 'hecustuser' ) )->get('/grapher/vlaninterface?id=1');
        $response->assertStatus(200);

        // customer admin user
        $response = $this->actingAs( $this->getCustAdminUser( 'hecustadmin' ) )->get('/grapher/vlaninterface?id=1');
        $response->assertStatus(200);

        // non-customer user
        $response = $this->actingAs( $this->getCustUser( 'imcustuser' ) )->get('/grapher/vlaninterface?id=1');
        $response->assertStatus(403);

        // non-customer admin user
        $response = $this->actingAs( $this->getCustAdminUser( 'imcustadmin' ) )->get('/grapher/vlaninterface?id=1');
        $response->assertStatus(403);

        // superadmin
        $response = $this->actingAs( $this->getSuperUser() )->get('/grapher/vlaninterface?id=1');
        $response->assertStatus(200);
    }
    ...
    public function testWebCustUserAccess()
    {
        ...
        $response = $this->actingAs( $this->getCustUser() )->get('/grapher/vlaninterface?id=1');
        $response->assertStatus(200);

        $response = $this->actingAs( $this->getCustAdminUser() )->get('/grapher/vlaninterface?id=1');
        $response->assertStatus(200);
