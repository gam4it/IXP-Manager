 ...
 */
class VirtualInterfaceApiAccessTest extends Access
{
    ...
    public function testApiPublicAccess()
    {
        ...
        $response = $this->get('/grapher/virtualinterface?id=1');
    ...
    public function testWebOwnUserCustUserAccess()
    {
        ...
        // customer user
        $response = $this->actingAs( $this->getCustUser( 'hecustuser' ) )->get('/grapher/virtualinterface?id=1');
        $response->assertStatus(200);

        // customer admin user
        $response = $this->actingAs( $this->getCustAdminUser( 'hecustadmin' ) )->get('/grapher/virtualinterface?id=1');
        $response->assertStatus(200);

        // non-customer user
        $response = $this->actingAs( $this->getCustUser( 'imcustuser' ) )->get('/grapher/virtualinterface?id=1');
        $response->assertStatus(403);

        // non-customer admin user
        $response = $this->actingAs( $this->getCustAdminUser( 'imcustadmin' ) )->get('/grapher/virtualinterface?id=1');
        $response->assertStatus(403);

        // superadmin
        $response = $this->actingAs( $this->getSuperUser() )->get('/grapher/virtualinterface?id=1');
        $response->assertStatus(200);
    }
    ...
    public function testWebCustUserAccess()
    {
        ...
        $response = $this->actingAs( $this->getCustUser() )->get('/grapher/virtualinterface?id=1');
        $response->assertStatus(200);

        $response = $this->actingAs( $this->getCustAdminUser() )->get('/grapher/virtualinterface?id=1');
        $response->assertStatus(200);
